...
with DAG(
        dag_id='1_hello_airflow', # unique identifier for the DAG
        schedule_interval=None, # somente via trigger manual
        start_date=datetime(2024, 6, 1), # start date for the DAG (June 1, 2024)
        description='Pipe de Hello World para validar o ambiente do treinamento.', # description of the DAG
    ...
    task_exec_hello_world = PythonOperator(
        task_id='task_exec_hello_world', # unique identifier for the task
        provide_context=True, # whether to provide context variables to the callable function
        python_callable=exec_hello_world, # callable function to be executed by the task
